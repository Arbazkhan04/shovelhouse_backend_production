    ...
    try {
        ...
        if (user) {
            ...
            if (user.userRole === 'houseOwner') {
                ...
                res.status(StatusCodes.CREATED).json({ 
                    user: { id: user._id, role: user.userRole, chargesEnabled: user.chargesEnabled, stripeAccountId: user.stripeAccountId, latitude: user.latitude, longitude: user.longitude,referralCode: user.referralCode },   
                    token 
                });
